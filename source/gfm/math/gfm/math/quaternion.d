...
struct Quaternion(T)
{
    public
    {
		...
		@nogc Vector!(T, 3) opBinary(string op : "*")(Vector!(T, 3) vec) pure const nothrow {
			auto inv = this.inversed;

			auto q = Vector!(T, 3)(inv.x, inv.y, inv.z);

			auto t = 2 * q.cross(vec);
			auto result = vec + inv.w * t + q.cross(t);
